...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  if($_POST['action'] == "add" && !empty($_POST['key']) && !empty($_POST['reaction'])){
    ...
    db_query_no_result($db, "INSERT INTO reactions VALUES ('$key', '$reaction', '$frequency', '$timeout')");
  }

  if($_POST['action'] == "del" && !empty($_POST['key'])){
    $key = addslashes(trim($_POST['key']));
    db_query_no_result($db, "DELETE FROM `reactions` WHERE `key` = '$key'");
  }
  ...
  if($_POST['action'] == "edit" && !empty($_POST['key'])){
    ...
    db_query_no_result($db, "UPDATE `reactions` SET `reaction` = '$reaction', `frequency` = '$frequency', `timeout` = '$timeout' WHERE `key` = '$key'");
  }
...
// Count
$count = db_query($db, "SELECT COUNT(`key`) as value FROM reactions")['value'];
