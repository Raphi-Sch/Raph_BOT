...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($_POST['action'] == "add" && !empty($_POST['form_key']) && !empty($_POST['form_nation']) && !empty($_POST['form_tier']) && !empty($_POST['form_type']) && !empty($_POST['form_name'])) {
        $key = strtolower(sanitise_input($db, $_POST['form_key']));
        ...
        db_query_no_result($db, "INSERT INTO tanks VALUES ('$key', '$nation', '$tier', '$name', '$mark', '$max_dmg', '$note', '$type')");
    }

    if ($_POST['action'] == "del") {
        $key = sanitise_input($db, $_POST['key']);
        db_query_no_result($db, "DELETE FROM tanks WHERE tanks.key='$key'");
    }

    if ($_POST['action'] == "edit") {
        $key = sanitise_input($db, $_POST['swal-key']);
        ...
        db_query_no_result($db, "UPDATE `tanks` SET `name` = '$name', `mark` = '$mark', `max_dmg` = '$dmg', `note` = '$note' WHERE `key` = '$key'");
    }
...
// List tank
$HTML = "";
$result = db_query_raw($db, "SELECT * FROM tanks ORDER BY `key` ASC");
...
// Count tank
$count = db_query($db, "SELECT COUNT(`key`) as value FROM tanks")['value'];
...
<body>
    ...
    <script>
        ...
        function add_entry() {
            Swal.fire({
                ...
                html: "<form id='swal-form' method='post'>" +
                    ...
                    // Key
                    "<label>Tank key</label><input type='text' class='form-control' name='form_key' required></br>" +
                    // Name
        ...
        function edit_entry(key) {
            var tank = document.getElementById(key).children;
            ...
            Swal({
                ...
                html: "<form id='swal-form' method='post' class='form-vertical'>" +
                    ...
                    "<input type='hidden' name='swal-key' value='" + key + "'>" +
